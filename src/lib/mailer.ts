...
function smtpConfig(): SMTPTransport.Options | null {
  ...
  const port = Number.parseInt(process.env.EMAIL_SERVER_PORT || "465", 10);
  const hasSecureEnv = process.env.EMAIL_SERVER_SECURE;
  const secure =
    hasSecureEnv !== undefined ? hasSecureEnv !== "false" : port === 465;
  ...
  const tlsReject = process.env.EMAIL_SERVER_TLS_REJECT_UNAUTHORIZED;
  if (tlsReject !== undefined) {
    config.tls = { rejectUnauthorized: tlsReject !== "false" };
  }
...
export async function sendEmail({ to, subject, html, text, from }: MailInput) {
  ...
  if (transporter && !delivered) {
    try {
      ...
      console.warn("[mailer] SMTP failed:", error);
    }
  }
