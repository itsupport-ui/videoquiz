...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session || session.user.role !== "ADMIN") return NextResponse.json({ message: "Forbidden" }, { status: 403 });
  const { id } = await params;
  // Get the module to be deleted to know its order
  const moduleToDelete = await (prisma as any).mainModule.findUnique({ where: { id: Number(id) } });
  if (!moduleToDelete) return NextResponse.json({ ok: true });
  const oldOrderIndex = moduleToDelete.orderIndex;
  ...
  // Auto-renumber remaining main modules to fill the gap
  await (prisma as any).mainModule.updateMany({ where: { orderIndex: { gt: oldOrderIndex } }, data: { orderIndex: { decrement: 1 } } });
